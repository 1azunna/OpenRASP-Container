...
// Configure mount to global RASP variable
...
// Specify the header name to be detected when detecting header injection, using lowercase uniformly
...
// Check whether eval_regex is legal
...
// Determine whether the parameter includes path traversal, which is stricter than path
...
// // Simple processing, also supports windows/linux
// var path2 = path.replaceAll('\\','/')
...
function validate_stack_java(stacks) {
    ...
    // v1.1.1 requires com.baidu.openrasp related classes to be filtered in the stack, because they are not implemented correctly, an extra reflection stack is generated, and compatibility is required to prevent false positives.
...
// parse the cookie from the string
...
// Combine the header, cookie, parameter, and json parameters in context.parameter, the key of header and cookie will be renamed
...
// Algorithm 4-ssrf_obfuscate
    ...
    // http://0x7f001
    //
    // The following obfuscation methods are not detected and are easy to be false
    ...
    plugin.register('sql', function (params, context) {
        ...
        function _run(values, name) {
            ...
            values.some(function (value) {
                // Do not process arrays of 3 dimensions and above
                if (typeof value != "string") {
                    return false
                }
                ...
                for(var i = 0, len = check_value.length; i <len; i++) {
                    ...
                    // If the database manager is allowed
        ...
        if (algorithmConfig.sql_policy.action !='ignore') {
            ...
            // Convert lowercase to avoid case bypass
...
// Algorithm 4: SQL regular expression
        ...
        // Join the cache, especially effective for prepared sql
        return clean
    })
...
plugin.register('directory', function (params, context) {
    ...
    // Algorithm 2-Check backdoors such as PHP kitchen knife
...
plugin.register('include', function (params, context) {
    ...
    // special agreement,    // include('file://XXX')
    // include('php://XXX')
...
// Algorithm 1: According to the stack, check whether it is a deserialization attack.// In theory, this algorithm does not have false positives
...
// Check the windows + SMB protocol to prevent the disclosure of NTLM information
